...
// Author a REAL AgentPreset for an enterprise agent: "core files" as context.
// The preset directory ($DSH_HOME/.agent-presets/<id>/) is discovered live by
// agentPresets, so the agent becomes startable immediately.
function slugify(name) {
  // ASCII-only slug; a Chinese-only name falls back to a short random tail.
  const s = String(name).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
  const slug = s || ('a' + Math.random().toString(36).slice(2, 5))
  return slug.slice(0, 40)
}

function buildCoreText(agent) {
  const caps = (agent.capabilities || []).join('；')
  const style = agent.style || '专业、务实'
  const profile = agent.profile || {}
  return [
    `# 我是谁（IDENTITY）`,
    `你是「${agent.name}」，企业的数字员工，由公司配置并为其工作。`,
    agent.description ? `\n## 职责说明\n${agent.description}` : '',
    caps ? `\n## 核心能力\n${caps}` : '',
    '',
    `# 个性与原则（SOUL）`,
    `风格：${style}。`,
    `原则：以事实为依据，不编造；无法确认时明确说明；涉及对外承诺或<敏感动作>前请求确认。`,
    '',
    `# 行为规则（AGENTS）`,
    `- 回答企业业务问题时，优先给出可验证、可追溯的依据。`,
    `- 用户提供的信息与系统事实冲突时，指出差异并说明采纳的版本。`,
    `- 使用技能时遵循对应技能文档的边界；技能外需求说明不支持。`,
    '',
    `# 记忆（MEMORY）`,
    `（初始空白。长期结论由使用中沉淀；此处不保存业务原始数据。）`,
    '',
    `# 用户（USER）`,
    profile.howToCall ? `- 称呼：${profile.howToCall}` : '',
    profile.language ? `- 偏好语言：${profile.language}` : '',
    profile.note ? `- 备注：${profile.note}` : '',
    profile.background ? `- 背景：${profile.background}` : '',
  ].filter((l) => l !== null).join('\n')
}

function buildPersonaText(agent) {
  const caps = (agent.capabilities || []).join('·')
  const style = agent.style || '专业、务实'
  const profile = agent.profile || {}
  const lines = [
    `你是「${agent.name}」，公司的数字员工。`,
    agent.description ? `职责：${agent.description}` : '',
    caps ? `核心能力：${caps}。` : '',
    `风格：${style}；以事实为依据，不编造。`,
  ]
  if (profile.howToCall) lines.push(`称呼用户为：${profile.howToCall}。`)
  if (profile.language) lines.push(`偏好语言：${profile.language}。`)
  if (profile.background) lines.push(`用户背景：${profile.background}。`)
  return lines.filter((l) => l).join('\n')
}

async function authorPreset(ctx, agent) {
  const presets = ctx.get('agentPresets')
  if (!presets) throw new Error('agentPresets 服务未挂载')
  const userRoot = path.join(DSH_HOME, '.agent-presets')
  let id = `agent-${slugify(agent.name)}`
  if (fs.existsSync(path.join(userRoot, id))) id = `${id}-${now().toString(36).slice(-4)}`
  const dir = path.join(userRoot, id)
  fs.mkdirSync(dir, { recursive: true, mode: 0o700 })

  // Base composition: the deployment's minimal preset, extended by our persona.
  let base = ''
  try {
    base = (await presets.read('minimal')) || ''
  } catch {
    base = ''
  }
  const personaBlock = [
    '# 企业智能体人设（核心文件上下文：IDENTITY/SOUL/AGENTS/USER 合帧）',
    '- id: persona',
    "  name: '@deepseek-ai/dsh-persona'",
    '  config:',
    '    text: >-',
    `      ${buildPersonaText(agent).split('\n').join('\n      ')}`,
  ].join('\n')
  fs.writeFileSync(path.join(dir, 'agent.cordis.yml'), personaBlock + (base ? '\n' + base : '') + '\n', 'utf8')

  // Metadata -> discovery name/description.
  fs.writeFileSync(path.join(dir, 'preset.yml'),
    `name: ${JSON.stringify(agent.name)}\ndescription: ${JSON.stringify(agent.description || '')}\norder: 90\n`, 'utf8')

  // Accio-style core files (context artifacts; kept for inspection/editing).
  const core = buildCoreText(agent)
  fs.writeFileSync(path.join(dir, 'IDENTITY.md'), core, 'utf8')
  fs.writeFileSync(path.join(dir, 'SOUL.md'), core, 'utf8')
  fs.writeFileSync(path.join(dir, 'AGENTS.md'), core, 'utf8')
  fs.writeFileSync(path.join(dir, 'MEMORY.md'), '（初始空白）\n', 'utf8')
  fs.writeFileSync(path.join(dir, 'USER.md'),
    [
      `# 用户信息`,
      profileOf(agent).howToCall ? `称呼：${profileOf(agent).howToCall}` : '',
      profileOf(agent).language ? `偏好语言：${profileOf(agent).language}` : '',
      profileOf(agent).note ? `备注：${profileOf(agent).note}` : '',
      profileOf(agent).background ? `背景：${profileOf(agent).background}` : '',
    ].filter(Boolean).join('\n') + '\n', 'utf8')
...
function profileOf(agent) {
  return agent.profile && typeof agent.profile === 'object' ? agent.profile : {}
}

// Every enterprise agent must own a real AgentPreset (authored core files),
// otherwise it cannot start a session and the UI falls back to showing DSH
// preset/mode names as if they were roles. Agents created before authoring
// existed (or whose preset dir was lost) get one here, idempotently.
async function ensureAgentPresets(record, ctx) {
  const missing = (record.agents || []).filter((a) => !a.presetId)
  if (!missing.length) return 0
  let authoredCount = 0
  for (const agent of missing) {
    try {
      const id = await authorPreset(ctx, agent)
      agent.presetId = id
      agent.authored = true
      agent.coreFiles = ['IDENTITY.md', 'SOUL.md', 'AGENTS.md', 'MEMORY.md', 'USER.md']
      agent.updatedAt = now()
      authoredCount += 1
    } catch (error) {
      console.error('[dsh-enterprise] authorPreset for agent failed:', agent?.id, String(error?.message ?? error))
    }
  }
  if (authoredCount > 0) saveRecord(record)
...
async function applyAction(record, action, payload, ctx) {
  ...
  switch (action) {
    case 'agent.create': {
      ...
      const authored = !presetId
      const finalPresetId = authored ? await authorPreset(ctx, agentDraft) : presetId
      record.agents.push({
        ...
        authored: authored,
        coreFiles: authored ? ['IDENTITY.md', 'SOUL.md', 'AGENTS.md', 'MEMORY.md', 'USER.md'] : [],
...
export function apply(ctx) {
  ...
  // Backfill authored presets for legacy/unbound agents as soon as possible
  // (never blocks the route; the GET handler also ensures before snapshot).
  ctx.get('agentPresets') // verify availability early, best-effort
  void (async () => {
    try {
      if (ctx.get('agentPresets')) await ensureAgentPresets(record, ctx)
    } catch (error) {
      console.error('[dsh-enterprise] ensureAgentPresets failed:', String(error?.message ?? error))
    }
  })()
  ...
  web.register({
    ...
    async handler(req, res) {      try {
        if (req.method === 'GET' || req.method === undefined) {
          try { await ensureAgentPresets(record, ctx) } catch { /* best-effort */ }
