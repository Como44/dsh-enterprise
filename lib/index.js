...
async function listSessions(ctx, limit = 40) {
  ...
  try {
    ...
    const stats = await sessionStatsOf(ctx, sorted)
...
async function sessionStatsOf(ctx, records) {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    try {
      ...
      if (projections && rec.live && live) {
        ...
        if (session) {
          const value = projections.stateOf(session, 'sessionStats')
          if (value) stats = pickStats(value)
        }
      }
      if (!stats && cache && rec.persisted) {
        try {
          const snap = cache.cachedSnapshot(rec.header)
          if (snap && snap.sessionStats) stats = pickStats(snap.sessionStats)
        } catch {
...
async function buildSnapshot(ctx, record) {
  ...
  // Usage aggregate: real per-agent numbers, structure-first member view.
  const byAgent = new Map()
  let totals = { sessions: sessions.length, withStats: 0, turns: 0, steps: 0, llmMs: 0, decodeTokens: 0, decodeMs: 0 }
  for (const s of sessions) {
    const key = s.agentPreset || '__default__'
    const acc = byAgent.get(key) ?? { agentPreset: key, sessions: 0, turns: 0, steps: 0, llmMs: 0, decodeTokens: 0 }
    acc.sessions += 1
    if (s.stats) {
      totals.withStats += 1
  ...
  const usage = {
    ...
    note: 'DSH 为本地单实例，成员级用量按演示映射展示；统计来自最近 40 个会话。',
  }
        ...
        if (req.method === 'POST') {
          ...
          req.on('end', async () => {
            try {
              ...
              await applyAction(record, action, parsed.payload, ctx)
              saveRecord(record)
              const state = await buildSnapshot(ctx, record)
              sendJson(res, 200, { ok: true, state })
            } catch (error) {
