...
async function conversationView(ctx, sessionId, afterSeq) {
  ...
  const seqNum = surface.capturedThroughSeq ?? 0
  ...
  try {
    agentPreset = surface.session && typeof surface.session.agentPreset === 'string' ? surface.session.agentPreset : null
  } catch {
    /* best-effort */
  }
  return { ok: true, seq: seqNum, title, agentPreset, messages }
}
