...
`
    ...
    function startTask(target, chip) {
      if (!target) { toast('还没有可用的智能体：请先创建或绑定一个预置角色', 'err'); return }
      if (target.kind === 'preset') {
        startPreset(target.preset.id, target.preset.name)
        if (chip) toast(`已选择场景：${chip}`)
        return
      }
      if (target.agent.presetId) {
        startPreset(target.agent.presetId, target.agent.name)
        if (chip) toast(`已选择场景：${chip}`)
        return
      }
    ...
    function NewTaskPage() {
      ...
      return el('div', { className: 'entm-page' },
        ...
        el('div', { className: 'entm-chips', style: { marginTop: 22 } },
          chips.map((c) => el('div', { key: c, className: 'entm-chip', onClick: () => startTask(target, c) }, el(Icon, { d: I.spark, size: 13 }), c)),
        ),
        el('div', { style: { marginTop: 22 } },
          el('button', { className: 'entm-btn primary', style: { padding: '10px 22px', fontSize: 14 }, onClick: () => startTask(target, null) },
            el(Icon, { d: I.play, size: 15 }), '开始任务'),
          el('p', { className: 'entm-note', style: { marginTop: 8 } }, '以该智能体（DSH 预置角色）创建新会话并在 DSH 中开始对话；所有任务都由智能体承接。'),
        ),
