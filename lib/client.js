...
/* DSH-native palette: consume the shell's live theme tokens (light/dark follows theme). */
.entm-overlay {
  --entm-bg: var(--dsw-alias-bg-base, #ffffff);
  --entm-panel: var(--dsw-alias-bg-layer-1, #f7f7f8);
  --entm-border: var(--dsw-alias-border-l1, #e3e3e6);
  --entm-border2: var(--dsw-alias-border-l2, #cfcfd4);
  --entm-text: var(--dsw-alias-label-primary, #1a1a1f);
  --entm-text2: var(--dsw-alias-label-secondary, #6b6b74);
  --entm-accent: var(--dsw-alias-brand-primary, #4d6bfe);
  --entm-accent-strong: var(--dsw-alias-brand-primary, #4d6bfe);
  --entm-accent-soft: color-mix(in srgb, var(--dsw-alias-brand-primary, #4d6bfe) 10%, transparent);
  --entm-danger: var(--dsw-alias-state-error-primary, #dc2626);
  --entm-ok: var(--dsw-alias-state-success-primary, #15803d);
  --entm-warn: var(--dsw-alias-state-warn-primary, #b45309);
  --entm-hover: color-mix(in srgb, var(--dsw-alias-label-primary, #1a1a1f) 6%, transparent);
  --entm-shadow: 0 1px 2px rgba(16, 18, 24, .05), 0 1px 6px rgba(16, 18, 24, .04);
  --entm-sidebar-fill: var(--dsw-specific-sidebar-fill, #f2f2f4);
}
.entm-sidebar { width: 226px; flex: none; display: flex; flex-direction: column; background: var(--entm-sidebar-fill); border-right: 1px solid var(--entm-border); padding: 14px 10px 12px; }
.entm-brand { display: flex; gap: 10px; align-items: center; padding: 2px 8px 12px; }
.entm-brand-mark { width: 28px; height: 28px; border-radius: 7px; background: var(--entm-accent); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 750; font-size: 14px; flex: none; }
...
.entm-card { background: var(--entm-panel); border: 1px solid var(--entm-border); border-radius: 8px; padding: 14px 13px 12px; display: flex; flex-direction: column; gap: 8px; cursor: pointer; transition: border-color .15s, box-shadow .15s; }
.entm-card:hover { border-color: var(--entm-border2); box-shadow: var(--entm-shadow); }
.entm-sessionsettings { width: 100%; border-top: 1px solid var(--entm-border); margin-top: 12px; padding-top: 12px; display: flex; gap: 14px; flex-wrap: wrap; }
.entm-error { position: fixed; inset: 0; z-index: 10005; background: var(--entm-bg, #fff); color: var(--entm-text, #1a1a1f); display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 14px; padding: 40px; text-align: center; }
.entm-error pre { max-width: 820px; max-height: 340px; overflow: auto; white-space: pre-wrap; word-break: break-word; font-size: 12px; line-height: 1.6; color: var(--entm-danger, #dc2626); background: var(--entm-panel, #f7f7f8); border: 1px solid var(--entm-border, #e3e3e6); border-radius: 8px; padding: 14px 16px; }
...
.entm-tag.warn { background: color-mix(in srgb, var(--entm-warn, #b45309) 12%, transparent); color: var(--entm-warn, #b45309); }
.entm-card-foot { display: flex; align-items: center; justify-content: space-between; margin-top: auto; }
.entm-agent-btn { width: 100%; border: 1px solid var(--entm-border); background: transparent; color: var(--entm-text); border-radius: 7px; padding: 7px 10px; font-size: 12.5px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; gap: 6px; transition: background .12s, border-color .12s; }
.entm-agent-btn:hover { background: var(--entm-hover); border-color: var(--entm-border2); }
...
.entm-banner { display: flex; align-items: center; gap: 14px; padding: 13px 14px; border-radius: 8px; background: var(--entm-panel); border: 1px solid var(--entm-border); margin-bottom: 18px; }
.entm-banner-ic { width: 36px; height: 36px; border-radius: 8px; background: var(--entm-accent-soft); color: var(--entm-accent); display: flex; align-items: center; justify-content: center; flex: none; }
...
.entm-chip { display: inline-flex; align-items: center; gap: 7px; padding: 6px 13px; border: 1px solid var(--entm-border); background: transparent; color: var(--entm-text2); border-radius: 999px; font-size: 12.5px; cursor: pointer; }
.entm-chip:hover { border-color: var(--entm-border2); color: var(--entm-text); background: var(--entm-hover); }
...
.entm-newtask-frame { display: flex; gap: 26px; width: 100%; max-width: 1060px; margin: 0 auto; padding-top: 4vh; align-items: flex-start; }
.entm-newtask-side { width: 292px; flex: none; }
.entm-newtask { max-width: none; flex: 1; min-width: 0; margin: 0; padding-top: 0; display: flex; flex-direction: column; align-items: center; gap: 24px; }
...
.entm-composer-card { width: 100%; background: var(--entm-panel); border: 1px solid var(--entm-border); border-radius: 10px; box-shadow: var(--entm-shadow); padding: 14px; }
...
`
    ...
    function groupSessions(snap) {
      // Agent-first: tasks are grouped by the enterprise agent that owns the
      // session's preset; DSH preset/mode names NEVER appear as group headings
      // (modes are session settings, not roles). Unassigned sessions collapse
      // into one ad-hoc group.
      const agentMap = new Map()
      for (const a of (snap.agents || [])) if (a.presetId) agentMap.set(a.presetId, a)
      const groups = new Map()
      for (const row of (snap.sessions || [])) {
        const presetId = row.agentPreset || ''
        const agent = presetId ? agentMap.get(presetId) : null
        const key = agent ? 'agent:' + agent.id : '__adhoc__'
        if (!groups.has(key)) {
          groups.set(key, {
            key,
            name: agent ? agent.name : '临时任务',
    ...
    function TaskList() {
      ...
      const pickGroup = (g) => {
        store.page = 'newtask'
        store.newTaskAgentId = g.agent ? g.agent.id : ''
        emit()
      }
      const newTaskFor = async (g) => {
        if (!g.agent) { toast('临时任务不属于任何智能体：请先创建或选择一位企业智能体'); return }
        await startTaskEntry({ kind: 'agent', agent: g.agent }, '')
        refresh()
      }
      return el('div', null,
        ...
        groups.map((g) => {
          ...
          return el('div', { key: g.key },
            el('div', { className: 'entm-sidebar-task entm-task-group', style: { fontWeight: 650 }, onClick: () => pickGroup(g) },
              el('span', null, g.icon),
              ...
              el('span', { className: 'entm-task-new', title: '新建任务', onClick: (e) => { e.stopPropagation(); newTaskFor(g) } },
      ...
      return el(
        ...
        el('div', { className: 'entm-sidebar-foot' },
          self ? el('div', { className: 'entm-user' },
            el('div', { className: 'entm-user-dot', style: { background: self.color || 'var(--entm-accent)' } }, esc(self.name.slice(0, 1))),
            el('div', null,
    ...
    function NewTaskPage() {
      ...
      const [modeSel, setModeSel] = useState('follow') // DSH presets = session settings (模式)
      const [settingsOpen, setSettingsOpen] = useState(false) // 会话设置展开区
      useEffect(() => {
        let alive = true
      ...
      const agents = (snap.agents || []).filter((a) => a.presetId)
      const target = resolveTaskTarget(snap, s.newTaskAgentId)
      const chips = ['办公提效', '设计', '建站', '采购', '数据分析']
      const modePresets = (snap.presets || []).filter((p) => !p.broken)
      const modeLabel = (id) => {
        const p = modePresets.find((x) => x.id === id)
        return p ? p.name : id
      }
      const sendNow = () => {
        const text = draft.trim()
        setDraft('')
        startTaskEntry(target, text, modelSel, modeSel)
      }
      const heroBg = target && target.kind === 'agent' && target.agent.color ? target.agent.color + '22' : 'var(--entm-accent-soft)'
      const heroColor = target && target.kind === 'agent' && target.agent.color ? target.agent.color : 'var(--entm-accent)'
      const roleAgent = target && target.kind === 'agent' ? target.agent : null
      const defaultRole = roleAgent || agents[0] || null
      return el('div', { className: 'entm-newtask-frame' },
        el('div', { className: 'entm-newtask' },
          el('div', { className: 'entm-newtask-hero' },
            ...
            el('div', { className: 'entm-icon', style: { width: 52, height: 52, background: heroBg, color: heroColor } },
              el('span', null, esc(target && (target.kind === 'agent' ? target.agent.icon || '🤖' : '🛰') || '🤖'))),
            el('div', { style: { fontSize: 18, fontWeight: 700 } }, esc(target ? (target.kind === 'agent' ? target.agent.name : target.preset.name) : '默认角色')),
            el('div', { className: 'entm-sub', style: { margin: 0, maxWidth: 520 } }, esc(target ? (target.kind === 'agent' ? target.agent.description : target.preset.description || '企业默认智能体，可直接开始') : '选择企业智能体开始工作')),
          ),
          el('div', { className: 'entm-composer-card' },
            ...
            el('div', { className: 'entm-composer-toolbar' },
              ...
              el('button', { className: 'entm-btn sm', onClick: () => setSettingsOpen(!settingsOpen) },
                el(Icon, { d: I.sliders, size: 12 }), settingsOpen ? '收起设置' : '会话设置'),
              el('span', { style: { flex: 1 } }),
              el('button', { className: 'entm-send', title: '发送', onClick: sendNow, disabled: !draft.trim() },
                el(Icon, { d: I.play, size: 14 })),
            ),
            settingsOpen ? el('div', { className: 'entm-sessionsettings' },
              el('div', { className: 'entm-field' },
                el('label', null, '会话模式（DSH 预设 = 会话运行方式，非角色）'),
                el('select', { className: 'entm-select', value: modeSel, onChange: (e) => setModeSel(e.target.value) },
                  el('option', { value: 'follow' }, '跟随角色'),
                  modePresets.map((p) => el('option', { key: p.id, value: p.id }, p.name)),
                ),
              ),
              models.length > 0 ? el('div', { className: 'entm-field' },
                el('label', null, '模型'),
                el('select', { className: 'entm-select', value: modelSel, onChange: (e) => setModelSel(e.target.value) },
              ...
              el('div', { className: 'entm-field' },
                el('label', null, '权限'),
                el('p', { className: 'entm-note', style: { margin: 0 } }, '跟随 DSH 原生 allow/ask/deny，在每次动作与审批面板中生效。'),
              ),
            ) : null,
          ),
          el('div', { className: 'entm-chips', style: { justifyContent: 'center' } },
            chips.map((c) => el('div', { key: c, className: 'entm-chip', onClick: () => startTaskEntry(target, `请开始一项「${c}」相关工作`, modelSel, modeSel) }, el(Icon, { d: I.spark, size: 13 }), c)),
          ),
        ),
        el('div', { className: 'entm-newtask-side' },
          el('div', { className: 'entm-sec-title' }, '默认角色'),
          el('div', { className: 'entm-card' },
            el('div', { className: 'entm-card-top' },
              el('div', { className: 'entm-icon', style: { background: defaultRole && defaultRole.color ? defaultRole.color + '22' : 'var(--entm-accent-soft)', color: defaultRole && defaultRole.color ? defaultRole.color : 'var(--entm-accent)' } },
                el('span', null, esc(defaultRole ? defaultRole.icon || '🤖' : '🤖'))),
              el('div', { className: 'entm-card-title' }, esc(defaultRole ? defaultRole.name : '尚未创建智能体')),
            ),
            el('div', { className: 'entm-card-desc', style: { minHeight: 0 } }, esc(defaultRole ? defaultRole.description || '' : '创建企业智能体后，此处显示公司默认角色（仅一个）。')),
            defaultRole ? el('div', { className: 'entm-tags' },
              defaultRole.authored ? el('span', { className: 'entm-tag accent' }, '核心文件角色') : null,
              (defaultRole.skills || []).slice(0, 3).map((sk) => el('span', { key: sk, className: 'entm-tag' }, sk)),
            ) : null,
            defaultRole ? el(StartButton, { agent: defaultRole }) : null,
          ),
          el('p', { className: 'entm-note', style: { marginTop: 8 } },
            '公司默认一个角色；「切换智能体」可选择其他企业智能体。DSH 预置（标准/代码/极简等）只是新会话的运行模式，在工具栏「模式」中选择。'),
        ),
      )
    }
    ...
    function AgentCard({ agent }) {
      ...
      return el('div', { className: 'entm-card' + (store.page === 'newtask' && store.newTaskAgentId === agent.id ? ' entm-card-active' : ''), onClick: pick },
        el('div', { className: 'entm-card-top' },
          el('div', { className: 'entm-icon', style: { background: agent.color ? agent.color + '22' : 'var(--entm-accent-soft)', color: agent.color || 'var(--entm-accent)' } }, el('span', null, esc(agent.icon || '🤖'))),
    ...
    function AgentsPage() {
      ...
      return el('div', { className: 'entm-page' },
        ...
        s.agentsTab === 'teams' ? el(TeamsPanel, null) : el('div', null,
          ...
          agents.length === 0 ? el('div', { className: 'entm-empty' }, '还没有企业智能体：点击「添加智能体」创建（DSH 预置只是新会话的模式，见新任务页「模式」）。') : null,
        ),
      )
    }
    ...
    function AgentDrawer() {
      ...
      const body = el('div', { className: 'entm-drawer-body' },
        ...
        agent.authored && (agent.coreFiles || []).length > 0 ? el('div', null,
          el('div', { className: 'entm-sec-title', style: { marginBottom: 6, fontSize: 13 } }, '核心文件（上下文）'),
          el('div', { className: 'entm-tags' }, agent.coreFiles.map((f) => el('span', { key: f, className: 'entm-tag accent', title: '该智能体的 AgentPreset 目录：' + f + ' 已作为上下文加载' }, f))),
        ) : null,
        el('div', { className: 'entm-kv' },
    ...
    function DrawerShell({ title, icon, color, body }) {
      return el('div', { className: 'entm-drawer' },
        el('div', { className: 'entm-drawer-head' },
          icon ? el('div', { className: 'entm-icon', style: { background: color ? color + '22' : 'var(--entm-accent-soft)', color: color || 'var(--entm-accent)' } }, el('span', null, icon)) : null,
    ...
    async function startTaskEntry(target, text, modelKey, modeKey) {
      if (!target) { toast('还没有可用的智能体：请先创建或绑定一个预置角色', 'err'); return }
      const rolePresetId = target.kind === 'preset' ? target.preset.id : target.agent.presetId
      if (!rolePresetId) {
        toast('该智能体未绑定 DSH 预置角色：请先绑定（详情 → 保存绑定），或直接选择预置角色。')
        return
      }
      const useMode = modeKey && modeKey !== 'follow' ? modeKey : null
      if (useMode && target.kind === 'agent' && target.agent.authored) {
        toast(`以会话模式「${useMode}」运行（本次不加载角色人设，模式=会话设置）`)
      }
      const presetId = useMode || rolePresetId
      try {
        const sessionId = await createSession(presetId)
        if (!sessionId) { toast('会话创建失败', 'err'); return }
    ...
    async function wizardSubmit() {
      ...
      if (ok) {
        store.wizard = null
        toast(presetId
          ? `智能体「${d.name.trim()}」创建成功，已绑定预置角色，可开始对话`
          : `智能体「${d.name.trim()}」创建成功：已生成核心文件与 Agent 角色，可直接开始对话`)
        emit()
      }
    }
          ...
          el('tbody', null, (snap.members || []).map((m) =>
            el('tr', { key: m.id },
              el('td', null, el('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
                el('div', { className: 'entm-user-dot', style: { background: m.color || 'var(--entm-accent)' } }, esc(m.name.slice(0, 1))),
    ...
    // -------------------------------------------------------------- overlay
    class EntmErrorBoundary extends React.Component {
      constructor(props) {
        super(props)
      ...
      componentDidCatch(error, info) {
        try { console.error('[dsh-enterprise] render error', error, info) } catch (_) {}
      }
      render() {
        if (this.state.error) {
          const msg = String(this.state.error && this.state.error.message ? this.state.error.message : this.state.error)
          return el('div', { className: 'entm-error' },
            el('div', { style: { fontWeight: 700, fontSize: 15 } }, '企业模式界面渲染出错（已拦截）'),
            el('pre', null, msg),
            el('button', {
              className: 'entm-btn primary',
              onClick: () => { try { store.open = false; store.snap = null } catch (_) {} emit() },
            }, '返回 DSH'),
          )
        }
        return el(React.Fragment, null, this.props.children)
      }
    }
    ...
    function OverlayRoot() {
      ...
      return el(EntmErrorBoundary, null,
        el('div', { className: 'entm-overlay' },
          el(Sidebar, null),
