        ...
        el('div', { className: 'entm-newtask' },
          ...
          el('div', { className: 'entm-composer-card' },
            ...
            el('div', { className: 'entm-composer-toolbar' },
              ...
              el('select', { className: 'entm-select', style: { maxWidth: 170 }, value: modeSel, onChange: (e) => setModeSel(e.target.value), title: '模式 = 新会话的运行方式（DSH 预设，非角色）' },
                el('option', { value: 'follow' }, '模式 · 跟随角色'),
                modePresets.map((p) => el('option', { key: p.id, value: p.id }, '模式 · ' + p.name)),
              ),
              models.length > 0 ? el('select', { className: 'entm-select', style: { maxWidth: 200 }, value: modelSel, onChange: (e) => setModelSel(e.target.value) },
