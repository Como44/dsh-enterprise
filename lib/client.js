...
.entm-msg { display: flex; }
.entm-msg.user { justify-content: flex-end; }
.entm-msg.assistant { justify-content: flex-start; }
.entm-msg-bubble { max-width: 76%; padding: 9px 13px; border-radius: 12px; font-size: 13px; line-height: 1.6; white-space: pre-wrap; word-break: break-word; background: var(--entm-panel); border: 1px solid var(--entm-border); }
.entm-msg.user .entm-msg-bubble { background: var(--entm-accent); border-color: var(--entm-accent); color: #fff; }
.entm-msg-tool { font-size: 11.5px; color: var(--entm-text2); padding: 2px 4px; }
...
`
      ...
      return el('div', null,
        ...
        groups.map((g) => {
          ...
          return el('div', { key: g.key },
            el('div', { className: 'entm-sidebar-task', style: { fontWeight: 650 } },
              el('span', null, g.icon),
    ...
    function NewTaskPage() {
      ...
      const sendNow = () => {
        ...
        setDraft('')
        startTaskEntry(target, text)
      }
      return el('div', { className: 'entm-page' },
        el('div', { style: { display: 'flex', alignItems: 'center', gap: 12 } },
          el('div', { className: 'entm-icon', style: { width: 46, height: 46, background: (target && (target.kind === 'agent' ? target.agent.color || '#00a573' : '#00a573')) + '22', color: target && (target.kind === 'agent' ? target.agent.color || '#00a573' : '#00a573') } },
            el('span', null, esc(target && (target.kind === 'agent' ? target.agent.icon || '🤖' : '🛰') || '🤖'))),
          el('div', null,
            el('div', { style: { fontSize: 17, fontWeight: 700 } }, esc(target ? (target.kind === 'agent' ? target.agent.name : target.preset.name) : '未选择智能体')),
            el('div', { className: 'entm-sub', style: { margin: 0 } }, esc(target ? (target.kind === 'agent' ? target.agent.description : target.preset.description || 'DSH 预置角色，可直接开始') : '选择一位企业智能体开始工作')),
          ),
          el('button', { className: 'entm-btn', style: { marginLeft: 'auto' }, onClick: () => { store.agentSel = null; store.page = 'agents'; emit() } },
            el(Icon, { d: I.bot, size: 13 }), '切换智能体'),
        ),
        el('div', { className: 'entm-composer', style: { marginTop: 22 } },
          el('textarea', {
          ...
          el('div', { style: { display: 'flex', alignItems: 'center', gap: 10, marginTop: 10 } },
            el('button', { className: 'entm-btn primary', onClick: sendNow }, el(Icon, { d: I.play, size: 14 }), '开始任务'),
            el('span', { className: 'entm-caption' }, '以所选智能体（DSH 预置角色）直接在此对话，无需跳转'),
          ),
        ),
        el('div', { className: 'entm-chips', style: { marginTop: 14 } },
          chips.map((c) => el('div', { key: c, className: 'entm-chip', onClick: () => startTaskEntry(target, `请开始一项「${c}」相关工作`) }, el(Icon, { d: I.spark, size: 13 }), c)),
        ),
    ...
    function ChatPage() {
      ...
      return el('div', { className: 'entm-page entm-chat' },
        el('div', { style: { display: 'flex', alignItems: 'center', gap: 10, paddingBottom: 10, borderBottom: '1px solid var(--entm-border)' } },
        ...
        el('div', { className: 'entm-chat-scroll', ref: scrollRef },
          ...
          chat.messages.map((m, i) =>
            m.kind === 'tool'
              ? el('div', { key: i, className: 'entm-msg-tool' }, esc(m.text))
              : el('div', { key: i, className: 'entm-msg ' + (m.kind === 'user' ? 'user' : 'assistant') },
                  el('div', { className: 'entm-msg-bubble' }, esc(m.text)),
                ),
          ),
          s.chatSending ? el('div', { className: 'entm-msg assistant' }, el('div', { className: 'entm-msg-bubble entm-msg-pending' }, '…')) : null,
        ),
        el('div', { className: 'entm-composer' },
          ...
          el('div', { style: { display: 'flex', alignItems: 'center', gap: 10, marginTop: 10 } },
            ...
            el('span', { className: 'entm-caption' }, '会话保持在此模式内；退出企业模式后对话仍可在 DSH 中找到')),
        ),
      )
    }
    ...
    function TeamEditorModal() {
      ...
      const editing = s.teamEditor.mode === 'edit' && snap ? (snap.teams || []).find((t) => t.id === s.teamEditor.id) : null
      const [name, setName] = useState(editing ? editing.name : '')
      const [desc, setDesc] = useState(editing ? editing.description || '' : '')
      const [tl, setTl] = useState(editing ? editing.tlPresetId || '' : '')
      const [members, setMembers] = useState(editing ? (editing.memberPresetIds || []) : [])
    ...
    async function startTaskEntry(target, text) {
