...
`
    ...
    function Sidebar() {
      ...
      const recent = snap ? (snap.sessions || []).slice(0, 6) : []
      const navMain = [
      ...
      const navMgmt = [
        ...
        { id: 'connections', label: '连接', icon: 'lock' },
      ...
      const item = (nav) =>
        el('div', {
          ...
          onClick: () => { store.page = nav.id; store.agentSel = null; emit() },
      ...
      return el(
        ...
        el('div', { className: 'entm-nav' },
          navMain.map(item), el('div', { className: 'entm-group' }, 'Organization'),
          navMgmt.map(item),
          recent.length > 0 ? el('div', null,
            el('div', { className: 'entm-group' }, '最近任务'),
            recent.map((row) =>
              el('div', { key: row.id, className: 'entm-sidebar-task', title: esc(row.title || '未命名会话'), onClick: () => openSession(row.id) },
    ...
    function HomePage() {
      ...
      const sessions = (snap.sessions || []).slice(0, 8)
      const skills = (snap.skills || []).slice(0, 5)
      const recommended = (snap.assets || []).filter((a) => a.recommended === true).slice(0, 6)
      const hourNow = new Date().getHours()
      const greet = hourNow < 6 ? '夜深了' : hourNow < 12 ? '早上好' : hourNow < 18 ? '下午好' : '晚上好'
      return el('div', { className: 'entm-page' },
        el('h1', { className: 'entm-h1' }, `${greet}，回到 ${esc(snap.org ? snap.org.name : '企业')}`),
        el('p', { className: 'entm-sub' }, '今天想让哪一位智能体开始工作？企业已经为你准备好的 Agent 与能力都在这里。'),

        el('div', { className: 'entm-sec' },
          el('div', { className: 'entm-sec-head' },
            el('span', { className: 'entm-sec-title' }, '我的智能体'),
            el('span', { className: 'entm-sec-link', onClick: () => { store.page = 'agents'; emit() } }, '查看全部 →'),
          ),
          el('div', { className: 'entm-grid' },
            agents.slice(0, 4).map((a) => el(AgentCard, { key: a.id, agent: a })),
            presets.slice(0, 2).map((p) => el(PresetCard, { key: p.id, preset: p })),
          ),
        ),

        el('div', { className: 'entm-sec' },
          el('div', { className: 'entm-sec-head' },
            el('span', { className: 'entm-sec-title' }, '最近工作'),
            el('span', { className: 'entm-sec-link', onClick: close }, '在 DSH 中打开 →'),
          ),
          sessions.length === 0
            ? el('div', { className: 'entm-empty' }, '还没有工作记录。从上面的智能体开始一个任务吧。')
            : el('div', { className: 'entm-list' }, sessions.map((row) => el(SessionRow, { key: row.id, row }))),
        ),

        el('div', { className: 'entm-sec' },
          el('div', { className: 'entm-sec-head' }, el('span', { className: 'entm-sec-title' }, '快速开始')),
          el('div', { className: 'entm-chips' },
            el('div', { className: 'entm-chip', onClick: close }, el(Icon, { d: I.spark, size: 13 }), '开始对话'),
            el('div', { className: 'entm-chip', onClick: () => { store.page = 'usage'; emit() } }, el(Icon, { d: I.chart, size: 13 }), '数据分析'),
            el('div', { className: 'entm-chip', onClick: close }, el(Icon, { d: I.box, size: 13 }), '文件整理'),
            el('div', { className: 'entm-chip', onClick: () => { store.page = 'agents'; emit() } }, el(Icon, { d: I.bot, size: 13 }), '开发任务'),
            el('div', { className: 'entm-chip', onClick: () => { store.page = 'skills'; emit() } }, el(Icon, { d: I.spark, size: 13 }), '企业技能'),
          ),
          el('div', { className: 'entm-grid', style: { marginTop: 12 } },
            el(QuickCard, { key: 'ask', title: 'Ask an Agent', desc: '随意提问，DSH 会按默认角色回答', icon: 'spark', onClick: close }),
            el(QuickCard, { key: 'create', title: 'Create Agent', desc: '30 秒创建你的企业智能体（五步向导）', icon: 'plus', onClick: () => { store.wizard = { step: 1, draft: wizardDraft() }; emit() } }),
            el(QuickCard, { key: 'start', title: 'Start Task', desc: '从预置角色开始一次任务（打开会话）', icon: 'play', onClick: () => { store.page = 'agents'; emit() } }),
            el(QuickCard, { key: 'ws', title: 'Open Workspace', desc: '选择工作区继续工作', icon: 'box', onClick: () => { store.workspacesOpen = true; emit() } }),
          ),
        ),

        (recommended.length > 0 || skills.length > 0) ? el('div', { className: 'entm-sec' },
          el('div', { className: 'entm-sec-head' },
            el('span', { className: 'entm-sec-title' }, '企业推荐能力'),
            el('span', { className: 'entm-sec-link', onClick: () => { store.page = 'assets'; emit() } }, '查看资产 →'),
          ),
          el('div', { className: 'entm-list' },
            (recommended.length > 0 ? recommended : skills.map((sk) => ({ id: 'skill:' + sk.name, refType: 'skill', name: sk.name, description: sk.description }))).map((r) =>
              el('div', { key: r.id, className: 'entm-row', onClick: () => { store.page = 'assets'; emit() } },
                el('span', { style: { display: 'flex' } }, el(Icon, { d: I.spark, size: 14 })),
                el('div', { className: 'entm-row-main' },
                  el('div', { className: 'entm-row-title' }, esc(r.name)),
                  el('div', { className: 'entm-row-sub' }, esc(r.description || ''))),
                el('span', { className: 'entm-tag accent' }, r.refType === 'skill' ? '技能' : r.refType === 'agent' ? '智能体' : '插件'),
              ),
            ),
          ),
        ) : null,
    ...
    function AgentCard({ agent }) {
      return el('div', { className: 'entm-card', onClick: () => { store.agentSel = { kind: 'agent', id: agent.id }; emit() } },
    ...
    function PresetCard({ preset }) {
      return el('div', { className: 'entm-card', onClick: () => { store.agentSel = { kind: 'preset', id: preset.id }; emit() } },
        el('div', { className: 'entm-card-top' },
    ...
    function QuickCard({ title, desc, icon, onClick }) {
      return el('div', { className: 'entm-card', onClick },
        el('div', { className: 'entm-card-top' },
          el('div', { className: 'entm-icon', style: { background: 'var(--entm-accent-soft)', color: 'var(--entm-accent)' } }, el(Icon, { d: I[icon] })),
          el('div', { className: 'entm-card-title' }, title)),
        el('div', { className: 'entm-card-desc' }, desc))
    }

    function SessionRow({ row }) {
      return el('div', { className: 'entm-row', onClick: () => openSession(row.id) },
    ...
    async function startPreset(presetId, name) {
      try {
        ...
        const sessionId = result && result.ok === true && result.value ? result.value.sessionId : null
        if (!sessionId) {
          const msg = result && result.error ? result.error.message : '会话创建失败'
          toast(String(msg || '未知错误'), 'err')
          return
        }
        const sessionsFace = ctxGet.modules.sessions
        if (sessionsFace && typeof sessionsFace.open === 'function') sessionsFace.open(sessionId)
        store.open = false
        emit()
      } catch (error) {
    ...
    function MembersModal() {
      ...
      const submit = async () => {
        ...
        if (ok) { toast('成员已添加'); store.membersOpen = false; emit() }
      }
    ...
    function SettingsPage() {
      ...
      const snap = s.snap
      if (!snap) return el(Loading, null)
      const [name, setName] = useState(snap.org ? snap.org.name : '')
      const [desc, setDesc] = useState(snap.org ? snap.org.description || '' : '')
      const budget0 = snap.org && snap.org.budget ? snap.org.budget : { period: 'month', limitCny: null, alertPercent: 80 }
      const [limitCny, setLimitCny] = useState(budget0.limitCny != null ? String(budget0.limitCny) : '')
      const [alertPercent, setAlertPercent] = useState(String(budget0.alertPercent != null ? budget0.alertPercent : 80))
      const saveOrg = async () => {
    ...
    function WorkspacesModal() {
      ...
      return el('div', { className: 'entm-modal-mask', onClick: () => { store.workspacesOpen = false; emit() } },
        el('div', { className: 'entm-modal', onClick: (e) => e.stopPropagation() },
          el('h3', null, '打开工作区'),
          rows.length === 0 ? el('div', { className: 'entm-empty' }, '尚无可用的工作区。请在 DSH 中先创建工作区。') :
    ...
    async function openWorkspace(w) {
      ...
      try {
        if (workspacesFace && typeof workspacesFace.connectWorkspace === 'function') {
          await workspacesFace.connectWorkspace(w.id)
        }
    ...
    function OverlayRoot() {
      ...
      const pageEl =
        ...
        page === 'settings' ? el(SettingsPage, null) : el(HomePage, null)
