...
**Agent-first 会话模式（无首页）：所有任务都以智能体开始。**

```text
Enterprise（全屏工作台，DSH 原界面零改动）
├── 新任务     默认界面：选择智能体 → 输入问题/场景快捷入口 →「开始任务」（模式内直接对话，不跳转）
├── 会话       模式内 ChatPage：消息气泡 + 输入框 + 增量刷新（session.prompt 发送，session surface 读取）
├── 智能体     Agent 目录（全部/企业共享/个人）+ 五步创建向导
│   └── 团队   企业团队定义（TL + 成员角色）+「以团队开始」+ DSH agentTeams 运行时视图（成员/任务看板）
├── 插件 / 知识库（P1 占位）
├── 任务       侧边栏：按智能体分组的最近会话（组头收起/展开，点击在模式内打开）
└── 组织
    ├── 企业资产  智能体/技能/插件统一视图 + 个人⇄企业共享 + 推荐
    ├── 技能
    ├── 成员      Owner / Admin / Member（添加/删除/改角色）
    ├── 使用量    用量 + 真实成本（按模型）+ 企业额度
    ├── 连接      Provider / 默认模型（BYO-API 可见性）；外部账号 P1
    └── 设置      企业信息 · 企业额度 · DSH 原生设置映射（主题/语言/Provider/插件/角色，只读 + 入口指引）
```
