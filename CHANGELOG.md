...
- **根因**：侧边栏「任务」列表按会话的 DSH 预置分组，组标题直接显示预置显示名 → 出现「标准模式 28 项」「创造模式 12 项」——又把会话模式当成了角色智能体（客户反馈：模式还在）；
- **修复**：任务按 **企业智能体** 分组（agent-first）：组标题 = 智能体名称 + 图标，不再出现任何预置/模式名；未归属任何企业智能体的会话统一收进「临时任务」组；
- 点组标题 → 新任务页选中该智能体；组内 ✏️ 新建任务 → 以该智能体直接开任务（临时任务组提示先选择/创建智能体）。
